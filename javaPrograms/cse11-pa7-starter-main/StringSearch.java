...
class StringSearch {
    ...
    public static void main(String [] args) throws IOException{
        ...
        if (args.length == 2){
            ...
            String[] andCase = args[1].split("&");
            Query[] query = new Query[andCase.length];
            for (int i = 0; i < andCase.length; i++){
                query[i] = check.readQuery(andCase[i]);
            }
            for (int i = 0; i < lines.length; i++){
                if (check.matchesAll(query,lines[i])){
        ...
        if (args.length == 3){
            StringSearch check = new StringSearch();
            String[] queryAndCase = args[1].split("&");
            Query[] query = new Query[queryAndCase.length];
            String[] transformAndCase = args[2].split("&");
            Transform[] transform = new Transform[transformAndCase.length];
            for (int i = 0; i < transformAndCase.length; i++){
                transform[i] = check.readTransform(transformAndCase[i]);
            }
            for (int i = 0; i < queryAndCase.length;i++){
                query[i] = check.readQuery(queryAndCase[i]);
            }
            for (int i = 0; i < lines.length; i++){
                if (check.matchesAll(query,lines[i])){
                    System.out.println(check.applyAll(transform,lines[i]));
                }
            }
        }
    }
